...
parser.add_argument('--model', type=str, default="bi_lstm_attn", help='Kind of Model to be trained')
...
parser.add_argument('--hidden_sizes', nargs='+', type=int, default=[100, 50], help='Hidden layer sizes')
parser.add_argument('--seed', type=int, default=8687960, help='Tensorflow seed')
...
# TP = tf.math.count_nonzero(y_test_hat * y_test)
# TN = tf.math.count_nonzero((y_test_hat - 1) * (y_test - 1))
# FP = tf.math.count_nonzero(y_test_hat * (y_test - 1))
# FN = tf.math.count_nonzero((y_test_hat - 1) * y_test)

results = model.evaluate(X_test, y_test, batch_size=batch_size)
print("test loss, test acc:", results)
_, acc, TP, FP, FN, TN = results
precision = TP / (TP + FP)
recall = TP / (TP + FN)
